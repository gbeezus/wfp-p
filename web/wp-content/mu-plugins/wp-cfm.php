 ...
 * @return string
 */
function f1_change_config_path( string $config_path ): string {

	// Check for Pantheon configuration and configure WP-CFM configuration paths for Pantheon.
	$pantheon_config = __DIR__ . '/../../../pantheon.yml';
 ...
 *
 * @param array<mixed> $environments - Default is an empty array [] meaning multi environment config is disabled.
 *
 * @return array<mixed>
 */
function f1_enable_multi_env( array $environments ): array {
...
function f1_get_current_environment() {
	...
	// Check for Pantheon Environment variable and use that if present.
	if ( defined( 'PANTHEON_ENVIRONMENT' ) && ! empty( $_ENV['PANTHEON_ENVIRONMENT'] ) ) {
		return sanitize_text_field( $_ENV['PANTHEON_ENVIRONMENT'] );
	}
